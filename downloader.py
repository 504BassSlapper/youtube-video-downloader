...
from moove_file import move_files_to_subdirectory
...
def Download(link, isAudio):
    """Download

    Args:
        link (string): link of the playlist to download
        isAudio (bool): Download as videos = true / as audios = false
    """
    ...
    if len(playlist) != 0:
        for url in playlist:
            counter += 1
            if isAudio.lower().strip() == "n":
                DownloadSingleVideo(url)
                logger(playlist, counter)
            else:
                DownloadSingleAudio(url)

        return ("All playlist videos have been downloaded")
    else:
